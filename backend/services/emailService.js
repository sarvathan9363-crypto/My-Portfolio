...
/**
 * Notify owner that a new contact message arrived
 */
const sendContactNotificationToOwner = async ({ name, email, projectType, message }) => {
  try {
 ...
 */
const sendContactConfirmationToClient = async ({ name, email, projectType }) => {
  try {
    await transporter.sendMail({
      from: `"Sarvathan C" <${process.env.GMAIL_USER}>`,
      to: email,
...
/**
 * Notify owner that a new feedback was submitted
 */
const sendFeedbackNotificationToOwner = async ({ name, role, message, rating }) => {
